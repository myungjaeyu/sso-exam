...
const PORT = process.env.SP_PORT || 3002
...
app
    ...
    .get('/info', (req, res) => {
        ...
        const { sdp3_sso_token } = req.cookies

        if (!sdp3_sso_token) return res.redirect(`http://localhost:11666/login?redirect_url=http://localhost:${ PORT }${ req.url }`)

        const { id, email, first_name, last_name } = verificationToken(sdp3_sso_token)

        res.send(`
        <html>
            <h1>SP3 User Info ${ first_name } ${ last_name }</h1>
        </html>
        `)
    ...
    .get('/auth/sso', (req, res) => {
        ...
        verificationToken(token) && res
                                    .cookie('sdp3_sso_token', token, { httpOnly: true })
